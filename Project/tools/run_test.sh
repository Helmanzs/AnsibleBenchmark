...
TESTS_FILE="$PROJECT_ROOT/vars/test_types.yml"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $0 [-database <name>|*] [-test_type <name>|*] [-profile <name>] [-r <1..N>] [-i <inventory>] [-rapid]"
      exit 1
      ;;
  esac
done
  ...
  for (( run=1; run<=REPEAT; run++ )); do
    ...
    for test in "${TEST_TYPES_TO_RUN[@]}"; do
      ...
      ansible-playbook $ANSIBLE_VERBOSITY "$PLAYBOOK_PATH" \
        -i "$HOSTS_FILE" \
        -e "database=$db test_type=$test global_results_dir=$RESULTS_DIR run=$run profile=$PROFILE deployment_env=$HOST_GROUP"
    done
...
echo "  Results written to: $RESULTS_DIR/$PROFILE-$test.ndjson"
echo "============================================="