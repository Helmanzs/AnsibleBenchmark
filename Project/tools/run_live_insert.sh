...
HOST_GROUP="local"
HOSTS_FILE="$PROJECT_ROOT/inventory/${HOST_GROUP}"
PROFILES_FILE="$PROJECT_ROOT/vars/resource_profiles.yml"
...
Options:
  ...
  -limit       <db1,db2,...>   Exclude these databases when using -d '*'
  ...
  -rapid                        Run tests in rapid mode              (default: false)
Examples:
  # Fully interactive
  $0
  ...
  # Run all databases EXCEPT postgres and mysql
  $0 -d '*' -limit postgres,mysql -p medium -w 4 -dur 120 -b 1000
EOF
      exit 0
      ;;
    *)
...
# Build the list of databases to run
if [[ "$DATABASE" == "*" ]]; then
  DATABASES_TO_RUN=()
  # Only include databases NOT in the -limit list
  IFS=',' read -ra EXCLUDE <<< "$LIMIT_DBS"
  for db in "${DATABASES[@]}"; do
    skip=0
    for ex in "${EXCLUDE[@]}"; do
      [[ "$db" == "$ex" ]] && skip=1 && break
    done
    [[ $skip -eq 0 ]] && DATABASES_TO_RUN+=("$db")
  done
else
...
PLAYBOOK_PATH="$PROJECT_ROOT/tests/test.yml"
...
for db in "${DATABASES_TO_RUN[@]}"; do
  ...
  for (( run=1; run<=REPEAT; run++ )); do
    echo
    echo "========== DATABASE: $db | RUN $run/$REPEAT =========="

    OTHER_DBS=()
    for other in "${DATABASES[@]}"; do
      if [[ "$other" != "$db" ]]; then
        OTHER_DBS+=("$other")
      fi
    done
    ...
    if [[ ${#OTHER_DBS[@]} -gt 0 ]]; then
      echo
      echo ">>> Stopping all other databases (${#OTHER_DBS[@]} hosts)..."

      LIMIT_LIST=$(IFS=,; echo "${OTHER_DBS[*]}")

      ansible-playbook $ANSIBLE_VERBOSITY \
        "$PROJECT_ROOT/setup/stop_vm.yml" \
        -i "$HOSTS_FILE" \
        -e "target=databases" \
        --limit "$LIMIT_LIST"
    fi
    ...
    ansible-playbook $ANSIBLE_VERBOSITY "$PLAYBOOK_PATH" \
      ...
      -e "global_results_dir=$RESULTS_DIR" \
      -e "run=$run" \
      -e "profile=${PROFILE:-}" \
      -e "workers=$WORKERS" \
      -e "duration=$DURATION" \
      ...
      -e "deployment_env=$HOST_GROUP"

    EXIT_CODE=$?
    if [[ $EXIT_CODE -ne 0 ]]; then
      echo
      echo "WARNING: Playbook exited with code $EXIT_CODE for database '$db' on run $run." >&2
      echo "         Continuing with remaining runs/databases..." >&2
    fi

    echo
    echo ">>> Finished run $run for $db"

  done
done

echo ">>> Stopping all databases"
ansible-playbook $ANSIBLE_VERBOSITY "$PROJECT_ROOT/setup/stop_vm.yml" \
  -i "$HOSTS_FILE" \
  -e "target=databases"
...
echo "  Results written to: $RESULTS_DIR/$PROFILE-live_insert.ndjson"
echo "============================================="