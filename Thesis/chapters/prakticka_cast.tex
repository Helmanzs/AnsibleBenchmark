\chapter{Praktická část}\label{practical}
...
Obsahuje celkem 54 atributů různých datových typů. Převládají celočíselné (int) a~desetinné čísla (float), které reprezentují měřené nebo stavové veličiny, dále textové atributy (text) sloužící především jako identifikátory a~časové údaje (timestamp). 
Struktura dat je plochá, přičemž každý řádek představuje jeden záznam o~stavu vozidla v~konkrétním časovém okamžiku.
...
Kromě toho je dostupný také HTTP API přístup. 
\cite{influxdbdocs}
...
QuestDB je dostupný jako open-source projekt pod licencí Apache License 2.0 a dále i jako QuestDB Enterprise. Ta disponuje funkcemi, které přidávají možnost replikace dat, automatického zálohování, horizontální škálování, a podporu úložiště nad objektovým storage (např. S3, Azure Blob Storage nebo GCS).
...
Následují odstavce popisují~realizace a~návrh testovacího prostředí, které bylo využito pro porovnání jednotlivých databázových systémů.
V~rámci testovacího prostředí je definováno co nejvíce sjednocené prostředí, jako například stejná data a~stejné HW prostředky.
